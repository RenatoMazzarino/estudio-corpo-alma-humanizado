...
Data: 2026-03-02
Escopo: `apps/web` + integrações/regras operacionais (tenant, WhatsApp, Spotify, IA, booking público)
...
2. Alias legado de playlist Spotify por env antigo
- Arquivos:
...
- Resultado: fonte canônica = `NEXT_PUBLIC_ATTENDANCE_SPOTIFY_PLAYLIST_URL` (ou settings).

3. Alias legado de IA
- Arquivo: `apps/web/src/shared/ai/flora.ts`
- Removido:
  - `GEMINI_MODEL` (permanece `FLORA_MODEL`)
  - `GOOGLE_API_KEY` (permanece `GEMINI_API_KEY`)

4. Full-scan fallback de CPF (busca pública)
- Arquivo: `apps/web/app/(public)/agendar/[slug]/public-actions/clients.ts`
- Removido: consulta ampla de até 5000 clientes quando mapa de candidatos vinha vazio.
- Resultado: apenas busca indexada/candidatos do fluxo atual.

5. Legado de automação WhatsApp por env (`MODE` + allowlist)
- Arquivos:
...
- Resultado: habilitação por tenant passa a seguir settings do banco (`tenant_whatsapp_settings` / `settings`).

## Ajustes de env canônico aplicados

1. `apps/web/.env.local`
- removidos: `GEMINI_MODEL`, `WHATSAPP_AUTOMATION_MODE`, `WHATSAPP_AUTOMATION_ALLOWED_TENANT_IDS`
- adicionados: `WHATSAPP_AUTOMATION_GLOBAL_ENABLED`, `WHATSAPP_AUTOMATION_FORCE_DRY_RUN`

2. `.vercel/env-import/vercel-prod-and-dev-required.env`
- removidos: `GEMINI_MODEL`, `WHATSAPP_AUTOMATION_MODE`, `WHATSAPP_AUTOMATION_ALLOWED_TENANT_IDS`
- adicionados: `WHATSAPP_AUTOMATION_GLOBAL_ENABLED`, `WHATSAPP_AUTOMATION_FORCE_DRY_RUN`
...
- Arquivos: `apps/web/app/auth/*/route.ts`, `apps/web/src/modules/auth/dashboard-access.ts`
- Motivo: previne open redirect e mantém retorno seguro.

2. Defaults de parse/normalização
- Exemplos: parsing numérico/textual em pagamentos e repositórios.
- Motivo: robustez contra payload externo incompleto.

3. Fallback funcional da Flora (evolução)
- Arquivos: `apps/web/src/modules/attendance/evolution-format.ts` e uso em `checklist-evolution.ts`
- Motivo: continuidade quando IA estiver indisponível.

4. Fallback de UX para compartilhamento
- Arquivos de voucher/export.
- Motivo: compatibilidade em ambientes sem Web Share API completa.

5. Defaults explícitos de regra de negócio
...
- O alvo de limpeza foi legado/duplicidade operacional; fallback de segurança/resiliência foi preservado.
