-- G6: add locking + internal appointment RPC
...
BEGIN
  ...
  -- Avoid double booking for the same tenant/slot
  PERFORM pg_advisory_xact_lock(hashtext(v_tenant_id::text), hashtext(date_trunc('minute', start_time)::text));
...
BEGIN
  ...
  PERFORM pg_advisory_xact_lock(hashtext(p_tenant_id::text), hashtext(date_trunc('minute', start_time)::text));
