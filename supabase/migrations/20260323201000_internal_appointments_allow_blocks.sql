-- Allow internal bookings even when availability_blocks overlap.
-- Decision: internal flow warns in UI but must not be blocked by plantao/bloqueios.
...
BEGIN
  ...
  ELSIF is_home_visit AND (
    ...
    INSERT INTO public.client_addresses (
      ...
      v_client_id,
      p_tenant_id,
      COALESCE(p_address_label, 'Novo endereco'),
      false,
      v_address_cep,
  ...
  -- Intencionalmente sem bloqueio por availability_blocks para fluxo interno.
  -- O usuario recebe aviso no UI e pode prosseguir.
