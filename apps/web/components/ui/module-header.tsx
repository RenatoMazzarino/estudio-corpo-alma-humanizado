    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          ...
          <div className="min-w-0 text-2xl font-serif leading-tight text-studio-text">{title}</div>
          {subtitle && <div className="text-xs text-muted mt-1">{subtitle}</div>}
        </div>
        {rightSlot && <div className="shrink-0 flex items-start gap-2">{rightSlot}</div>}
      </div>
