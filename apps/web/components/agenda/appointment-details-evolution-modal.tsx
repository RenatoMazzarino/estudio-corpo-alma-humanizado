...
export function AppointmentDetailsEvolutionModal({
  ...
  return (
    <div className="absolute inset-0 z-20 flex items-center justify-center px-4">
      <button
        type="button"
        aria-label="Fechar edicao de evolucao"
        onClick={onClose}
        ...
        <div className="flex items-center justify-between border-b border-line wl-sheet-header-surface px-4 py-3">
          <div>
            ...
            <p className="text-[10px] font-extrabold uppercase tracking-widest text-white/80">
              Edite sem sair do pos-atendimento
            </p>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="wl-header-icon-button-strong flex h-8 w-8 items-center justify-center rounded-lg"
          >
            <X className="h-4 w-4" />
          </button>
        </div>

        <div className="space-y-3 p-4 wl-surface-modal-body">
          <div className="flex items-center justify-end">
            <button
              type="button"
              onClick={onStructure}
              disabled={!canStructure || !draft.trim() || structuring || saving || actionPending}
              className="flex h-9 w-9 items-center justify-center rounded-xl border border-studio-green/30 bg-paper text-studio-green disabled:opacity-50"
          ...
          <p className="text-[11px] font-semibold text-muted">
            {structuring
              ? "Flora esta organizando o texto..."
