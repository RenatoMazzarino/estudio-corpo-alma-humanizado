...
export function HubStage({ appointment, attendance, onOpenStage, onResumeSession, timerLabel, timerActive }: HubStageProps) {
  ...
  return (
    <main className="flex-1 overflow-y-auto no-scrollbar px-6 pt-6 pb-28 space-y-4">
      {timerActive && (
        <div className="bg-studio-light border border-line rounded-2xl px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className={`w-2 h-2 rounded-full ${attendance.timer_status === "running" ? "bg-red-500 animate-pulse" : "bg-orange-400"}`} />
            <div>
              <p className="text-[10px] font-extrabold uppercase tracking-widest text-muted">Sessão em andamento</p>
              <p className="text-lg font-black tabular-nums text-studio-text">{timerLabel ?? "00:00"}</p>
            </div>
          </div>
          <button
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between">
              <div className="text-lg font-serif font-bold text-studio-text">{card.title}</div>
              <StageStatusBadge status={status} />
            </div>
            <p className="mt-2 text-sm text-muted font-semibold leading-relaxed">{card.description}</p>
            <div className="mt-4 flex items-center justify-between gap-3 text-[10px] font-extrabold uppercase tracking-widest text-muted">
              <span className="inline-flex items-center gap-2 bg-studio-light text-studio-green px-3 py-2 rounded-full">
